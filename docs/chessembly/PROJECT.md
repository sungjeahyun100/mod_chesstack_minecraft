...
게임의 '상태'를 읽거나, 이후 활성화될 칸에 특별한 효과를 부여합니다.
...
- `summon(kind, dx, dy)`: (수식어) 이후 활성화되는 🔵칸에 "클릭 시 `(dx, dy)` 위치에 `kind` 기물을 소환" 액션을 부착합니다.
- `auto(dx, dy)`: (수식어) 이후 활성화되는 🔵칸에 "클릭 시 해당 기물에 `(dx, dy)` 방향 자동 이동(take-move 모드) 설정" 액션을 부착합니다.
- `auto-shift(dx, dy)`: (수식어) 이후 활성화되는 🔵칸에 "클릭 시 해당 기물에 `(dx, dy)` 방향 자동 이동(shift 모드) 설정" 액션을 부착합니다.
...
### 6. 히스토리식 (History Expressions)

게임의 이동 기록을 참조하는 조건식입니다.

- `history-moved(kind)`: (조건식) 해당 종류(`kind`)의 기물이 이동한 적 있으면 `true`를 반환합니다.
- `history-exists(fromX, fromY, toX, toY)`: (조건식) 특정 좌표 간 이동이 기록에 존재하면 `true`를 반환합니다.
